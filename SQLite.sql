...
SELECT COUNT(DISTINCT(customer_nodes.node_id)) as unique_nodes
FROM customer_nodes

-- 2. What is the number of nodes per region?

SELECT regions.region_name, COUNT (customer_nodes.node_id) as node_counts
from regions
join customer_nodes 
...
SELECT regions.region_name, COUNT(DISTINCT (customer_nodes.customer_id)) as customers_counts
from regions
join customer_nodes 
	on regions.region_id = customer_nodes.region_id
GROUP BY regions.region_name
...
SELECT AVG(DATEDIFF(day,start_date,end_date)) as average_reallocation_days
FROM customer_nodes
